# Current Server Database Export Script
# Exports data from the CURRENT server (sr-bft-w10-lp07\SQLEXPRESS1) to cad_db_current.db
...
log_filename = f'get_current_db_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(
    level=logging.INFO,
...
# Force connection to the CURRENT server (lp07)
CURRENT_SERVER_NAME = 'sr-bft-w10-lp07'  # CURRENT server hostname
...
if current_hostname == CURRENT_SERVER_NAME.lower():
    # Running on the current server - use local connection
...
MSSQL_CONN_STR = f"Driver={{SQL Server}};SERVER={CURRENT_SERVER};Database=ECADMASTER;Trusted_Connection=yes;"
...
SQLITE_DB_PATH = os.path.join(OUTPUT_DIR, 'cad_db_current.db')
SQLITE_DB_NAME = 'cad_db_current.db'

logger.info("=" * 60)
logger.info("CURRENT SERVER DATABASE EXPORT")
logger.info("=" * 60)
...
def export_tables_to_sqlite(mssql_engine):
    """Exports tables from current server to cad_db_current.db"""
    logger.info("Starting export from current server...")
    sqlite_engine = None
    try:
        ...
        with mssql_engine.connect() as cnxn:
            logger.info("[OK] Connected to current server")
            ...
            if recent_jobs_df.empty:
                logger.warning("No recent jobs found on current server.")
                return False
            
            recent_job_numbers = tuple(recent_jobs_df['NUMERO'].tolist())
            logger.info(f"[OK] Found {len(recent_job_numbers)} recent jobs on current server")
            ...
            logger.info("STEP 3: Exporting filtered tables from current server...")
        ...
        logger.info("=" * 60)
        logger.info("[SUCCESS] Current database created successfully")
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("STARTING CURRENT SERVER EXPORT")
    logger.info("=" * 60)
    logger.info(f"Log file: {log_filename}")
    
    # Try multiple connection methods
    # Current server uses SQLEXPRESS, try that first
    ...
    try:
        if successful_engine:
            ...
            if export_tables_to_sqlite(successful_engine):
                logger.info("[SUCCESS] Current database export completed")
            else:
                logger.error("[FAILED] Current database export failed")
        else:
